...
- 송신자와 수신자 사이의 **거리 증가**로 인해 전파 세기가 점점 약해짐.
    - 송신자(신호원) 근처에서 수신자(Target)가 움직일 때 거리 변화에 따라 발생.
...
- **장시간 동안** 평균적인 무선 신호 변화.
    ...
    - 거리 감쇠(Path Loss) + 큰 지형물 차폐(Shadowing)
...
- **단시간 동안** 신호가 급격하게 변하는 현상.
    ...
    - 짧은 시간 또는 작은 거리 이동에서도 생기는 **신호 세기 변동**.
        ...
        - 송신기에서 보낸 전파가 건물, 지면, 차량 등에 부딪혀 여러 경로로 수신기에 도달하는 현상
        - 수신 신호는 여러 개의 반사파·회절파·산란파가 시간 지연과 위상 차이를 가지고 합쳐짐
        - 이때 신호들이 서로 보강 간섭(강해짐) 또는 상쇄 간섭(약해짐)을 일으켜 신호 세기가 빠르게 변동함
