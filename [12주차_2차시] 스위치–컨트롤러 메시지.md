...
sequenceDiagram
    ...
    %% 1) h1이 첫 Ping Request 전송
    H1 ->> SW: (1) Ping Request (ICMP Echo Request)

    %% 2) 스위치: 룰 없음 → PacketIn
    SW ->> CT: (2) PacketIn<br/>reason=NO_MATCH<br/>buffer_id=3<br/>in_port=1<br/>src=h1 dst=h2

    %% 3) 컨트롤러: h1 MAC 학습, h2 위치 모름 → FLOOD 결정
    CT ->> CT: h1 MAC 학습 (port1)<br/>h2 위치 모름 → FLOOD 결정<br/>buffer_id=3 기억

    %% 4) FlowMod (FLOOD용, buffer_id는 사용하지 않음)
    CT -->> SW: (3) FlowMod (임시 FLOOD 룰)<br/>match(h1→h2, in_port=1)<br/>actions=FLOOD<br/>buffer_id=NO_BUFFER

    %% 5) PacketOut: 버퍼에 있는 첫 패킷 FLOOD
    CT ->> SW: (4) PacketOut<br/>buffer_id=3<br/>actions=FLOOD

    %% 6) 스위치: FLOOD → h2까지 전달
    SW ->> H2: (5) Ping Request FLOOD

    %% 7) h2가 Ping Reply 전송
    H2 ->> SW: (6) Ping Reply (ICMP Echo Reply)

    %% 8) 다시 룰 없음(유니캐스트 룰 미설치 상태) → 두 번째 PacketIn
    SW ->> CT: (7) PacketIn<br/>reason=NO_MATCH<br/>buffer_id=4<br/>in_port=2<br/>src=h2 dst=h1

    %% 9) 컨트롤러: h2 MAC 학습, 이제 h1·h2 위치 모두 앎
    CT ->> CT: h2 MAC 학습 (port2)<br/>이제 h1↔h2 유니캐스트 경로 설정

    %% 10) FlowMod #1 : h1 → h2 방향
    CT -->> SW: (8) FlowMod #1<br/>match(h1→h2, in_port=1)<br/>actions=output:port2<br/>buffer_id=NO_BUFFER

    %% 11) FlowMod #2 : h2 → h1 방향
    CT -->> SW: (9) FlowMod #2<br/>match(h2→h1, in_port=2)<br/>actions=output:port1<br/>buffer_id=NO_BUFFER

    %% 12) 지금 들어온 Reply 패킷은 PacketOut으로 한 번 보내줌
    CT ->> SW: (10) PacketOut<br/>buffer_id=4<br/>actions=output:port1

    %% 13) 스위치: Reply 전달
    SW ->> H1: (11) Ping Reply 전달

    Note over SW,H1: "이후 h1↔h2 트래픽은 설치된 FlowMod로 자동 처리됨 (PacketIn 없음)"
