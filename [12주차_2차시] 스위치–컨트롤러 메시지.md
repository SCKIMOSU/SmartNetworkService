...
flowchart TD
    ...
    A[h1 → h2 첫 ping 전송] --> B[스위치: FlowTable에 룰 없음<br/>→ PacketIn 생성<br/>reason = NO_MATCH<br/>buffer_id = 3<br/>data = 패킷 헤더 일부]

    B --> C[컨트롤러:<br/>- src MAC 학습<br/>- dst MAC 모름 → FLOOD 결정]

    C --> D[FlowMod 전송<br/>match = (in_port=1, src=h1, dst=h2)<br/>actions = output:FLOOD<br/>buffer_id = 3]

    D --> E[스위치 동작<br/>- buffer_id=3 패킷을 버퍼에서 꺼냄<br/>- FlowMod actions(FLOOD)대로 즉시 처리<br/>- PacketOut 필요 없음<br/>- Flow Table에 룰 설치 완료]
