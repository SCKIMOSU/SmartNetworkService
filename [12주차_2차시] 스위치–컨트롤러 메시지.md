# [12주차_2차시] Ping Request로 보는 스위치–컨트롤러 메시지

# **Ping Request → PacketIn → FlowMod → PacketOut (Sequence Diagram)**
...
sequenceDiagram
    ...
    Note over H1,CT: Ping Request → PacketIn → FlowMod → PacketOut 상세 시퀀스

    %% 1. Host → Switch : Ping Request
    H1 ->> SW: (1) Ping Request (ICMP Echo Request)<br/>Ethernet + IP + ICMP 패킷

    %% 2. Switch → Controller : PacketIn
    SW ->> CT: (2) PacketIn<br/>reason = NO_MATCH<br/>buffer_id = 3 (버퍼 저장)<br/>in_port = 1<br/>data = 패킷 헤더/일부 payload

    %% 3. Controller 내부 처리
    CT ->> CT: src MAC 학습 (h1 → in_port=1)<br/>dst MAC(h2) 모름 → FLOOD 결정

    %% 4. FlowMod 전송
    CT -->> SW: (3) FlowMod<br/>match(eth_src=h1, eth_dst=h2, in_port=1)<br/>actions=output:FLOOD<br/>buffer_id = NO_BUFFER<br/>(단지 룰만 설치)

    %% 5. PacketOut 전송
    CT ->> SW: (4) PacketOut<br/>buffer_id = 3<br/>actions = output:FLOOD<br/>data = 없음(스위치 버퍼 사용)

    %% 6. Switch는 buffer_id 3 패킷을 FLOOD 처리
    SW ->> H1: send frame (FLOOD)
    SW ->> H2: send frame (FLOOD)
    SW ->> ...: broadcast to all ports (except input port)

    Note over SW: Flow Table 설치 완료<br/>이후 동일 패킷은 PacketIn 없이 룰에 따라 자동 처리됨
