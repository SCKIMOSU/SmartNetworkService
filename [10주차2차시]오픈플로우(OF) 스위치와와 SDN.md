...
- 각 스위치 포트는 자체 MAC 주소를 가지고 있어야 함
    - 프레임 전송 시 출발지 MAC으로 식별 가능
    - LLDP(Link Layer Discovery Protocol)등의 프로토콜 통신 가능
    - OpenFlow 메시지에서 포트 식별용으로 사용 

---

## 연결된 장치의 MAC 주소는 서로 다름
...
- 출력 마지막에 있음
    - `LOCAL` 포트는 **스위치 자체를 대표하는 내부 인터페이스**
        - **스위치의 내부 관리용 MAC 주소**로, 외부로 트래픽이 직접 나가지는 않음.
...
# [4]  **스위치의 MAC 주소 사용과** 라우터의 IP 주소 사용
...
- 방화벽(FW) NFV 기능 자체 : 스위치(또는 별도의 가상 네트워크 기능 노드)에서 수행
- 컨트롤러 : 스위치가 방화벽을 수행하도록 **제어·오케스트레이션(지휘)**
    - 컨트롤러 : 지능(뇌) 역할
    - 스위치, NFV 노드 :  실행(손) 역할
...
- **물리 방화벽**(Hardware Firewall)은 전용 네트워크 보안 장비로
- 네트워크 트래픽을 **하드웨어 기반으로 필터링·제어**하여 내부 네트워크를 외부 공격으로부터 보호하기 위한 장비
