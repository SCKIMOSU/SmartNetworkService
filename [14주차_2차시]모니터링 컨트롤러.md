...
- 네트워크에서 Host 간 트래픽 흐름을 실시간 모니터링
- 포트 및 대역폭 통계 수집
- SDN 기반 네트워크 성능 분석
- 토폴로지 변화나 흐름 변화 감지
- 패킷/바이트 기반 트래픽량 분석
...
- **스위치 연결/해제 감지**
    - **datapath 목록 관리**
- **10초 주기 스레드로 전체 스위치에 StatsRequest(Flow/Port) 요청 전송**
- **FlowStatsReply/PortStatsReply를 받아 읽기 쉬운 형태로 출력**
- 기본 L2 스위칭 기능
    - 부모 클래스(SimpleSwitch13)가 수행
    - **SimpleSwitch13**
        - **MAC learning 스위치**
    - **SimpleMonitor**
        - **+ Flow/Port monitoring 기능 추가**

---

### 1. 클래스 선언 및 초기화

- 부모 클래스 `SimpleSwitch13` 의 기능(MAC 주소 학습 기반 스위칭)을 그대로 사용
...
- 부모 클래스 초기화 (스위칭 기능 포함)
- **datapath를 저장할 딕셔너리 생성**
    - 여러 스위치 관리 가능
- **hub.spawn(self._monitor)**
    - **10초 주기 모니터링 스레드 시작**
    - Ryu의 `hub`
        - green thread(경량 스레드) 구현
...
- 스위치가 다음 상태로 바뀔 때 호출됨
    - `MAIN_DISPATCHER`
        - OpenFlow 채널이 완전히 연결됨
    - `DEAD_DISPATCHER`
        - 연결이 끊어짐
...
- 모든 스위치(datapath)에 대해
    - **Flow Stats** + **Port Stats** 요청을 전송
    - 10초 대기 후 반복
...
- 컨트롤러가 FlowStatsReply 메시지를 수신했을 때 호출되는 핸들러
...
- priority==1 인 flow만 출력
    - SimpleSwitch13이 설치하는 flow rule의 우선순위가 1이기 때문에 이를 필터링
...
- 스위치가 컨트롤러에 연결
    - datapaths에 등록
- 컨트롤러의 모니터 스레드가 10초마다 Stats Request 전송
- 스위치가 FlowStatsReply / PortStatsReply 응답
- 컨트롤러는 이를 받아 사람이 읽기 쉬운 형태로 출력
- 기본 스위칭은 SimpleSwitch13가 처리
