...
- **`CreateThread()`를 호출했다고 해서 두 스레드가 동시에 바로 작동하는 것은 아님.**
    - **거의 동시에 실행될 가능성이 높음.**
...
- `CreateThread()`를 호출하면 Windows 커널은 다음 일을 수행
1. **스레드 객체 생성**
    
    → 커널 내부에서 스레드 컨트롤 블록(TCB)을 만들고,
    
    → 사용자 모드 스택과 커널 모드 스택을 할당함.
    
2. **시작 함수(MyThread)와 인자(arg) 등록**
    
    → 새 스레드가 실행될 때 호출할 함수 포인터(`LPTHREAD_START_ROUTINE`)와
    
    → 인자(`LPVOID arg`)를 내부적으로 저장함.
    
3. **스레드를 실행 준비 상태(RUNNABLE)로 등록**
    
    → 만약 `CREATE_SUSPENDED`가 아닌 경우,
    
    커널 스케줄러는 즉시 이 스레드를 “Ready Queue(준비 큐)”에 넣음.
    
4. **리턴**
    
    → `HANDLE`을 반환할 때, **스레드가 이미 실행 중일 수도 있고, 아직 스케줄링을 기다리고 있을 수도 있음**
...
- 첫 번째 호출 시 `MyThread()`를 위한 스레드가 커널에 등록되고, **운영체제 스케줄러가 바로 실행시킬 수도 있음.**
- 두 번째 스레드 생성 시에는 첫 번째 스레드가 이미 실행 중일 수도 있고, 아직 CPU를 받지 못했을 수도 있음.
    - 두 스레드가 거의 동시에 실행될 준비가 되지만, **정확히 동시에 실행되는지는 CPU 코어 수, 스케줄링 타이밍에 따라 다름.**

---

## 순서가 달라지는 이유

- 각 스레드가 `Sleep(1000)` 후 `printf`를 하는데, 1000ms가 끝나고 누가 먼저 깨어나 CPU를 얻는가는 완전히 스케줄러의 결정임.
    - 출력 순서가 매번 다르게 보임
    - **`hThread1`이 먼저 생성됐다고 해서 `Sleep(1000)` 후 꼭 먼저 깨어나지는 않음**
