...
---
- `inet_pton`을 쓰지 않고 직접 값을 넣는 경우
    - 만약 프로그래머가 문자열 `"192.168.0.1"`을 직접 파싱해서 `struct in_addr`에 `s_addr = 0xC0A80001;` (리틀엔디안 CPU 기준) 이런 식으로 집어넣는다면, 이 값은 **호스트 바이트 오더**로 저장된 것이지 **네트워크 바이트 오더**가 아님.
